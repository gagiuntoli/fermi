...
int ferirods(void) {
  ...
  while (pr) {
    ...
    pp = list_physe.head;
    while (pp) {
      if (!strcmp(((gmshP_t *)pp->data)->name,
    ...
    for (e = 0; e < mesh.nelems; e++) {
      if (((ps_t *)mesh.elems[e].prop)->gmshid == gmshid) {
        for (d = 0; d < 3; d++)
          ((ctrlrod_t *)pr->data)->p[d] =
              mesh.node[mesh.elems[e].nodel[0]].coor[d];
        for (d = 0; d < nproc; d++)
          error = MPI_Send((void *)((ctrlrod_t *)pr->data)->p, 3, MPI_DOUBLE, d,
                           0, FERMI_Comm);
        flag = 1;
        break;
      }
    ...
    if (!flag) {
      error = MPI_Recv((void *)((ctrlrod_t *)pr->data)->p, 3, MPI_DOUBLE, 1, 0,
                       FERMI_Comm, MPI_STATUS_IGNORE);
      if (error)
        return 1;
    }
...
int fersrods(double t) {
  /*fils the list xsa of the ctrlrod_t structure according to the value of
   * time*/
