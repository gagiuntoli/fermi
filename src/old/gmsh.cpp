...
int gmsh_read(char *file, char *efile, char *nfile, int rank, int dim, list_t *list_nodes, list_t *list_ghost,
              list_t *list_elemv, list_t *list_elems, list_t *list_phyce, int **loc2gold, int **loc2gnew, int **npp,
              int nproc) {
...
int gmsh_readnodes(char *file, int nproc, char *nfile, int rank, list_t *list_nodes) {
  ...
  while (fgets(buff1, MAXC, fm) != NULL) {
    ...
    if (strcmp(data, "$Nodes") == 0) {
      if (fgets(buff1, MAXC, fm) == NULL) return 1;
      sel = SN;
      skip = 1;
    } else if (strcmp(data, "$EndNodes") == 0) {
      if (nproc > 1) fclose(fn);
      fclose(fm);
      return 0;
    }
    if (sel == SN && !skip) {
      if (nproc > 1) {
        if (!fgets(buff2, MAXC, fn)) return 1;
        sscanf(buff2, "%d", &p);
        if (p < 0) return 1;
      } else {
        p = rank;
      ...
      if (p == rank) {
        ...
        if (list_insertlast(list_nodes, (void *)&node) != 0) return 1;
      }
    }
    if (skip) skip = 0;
  }
...
int gmsh_readelemv(char *file, int nproc, char *efile, int rank, int dim, list_t *list_elemv) {
  ...
  while (fgets(buff1, MAXC, fm) != NULL) {
    ...
    if (sel == SN && !skip) {
      data = strtok(NULL, " \n");
      if (!data) return 1;
      if (gmsh_isvol(atoi(data), dim) < 0) return 1;
      if (gmsh_isvol(atoi(data), dim)) {
        if (nproc > 1) {
          if (fgets(buff2, MAXC, fe) == NULL) return 1;
          sscanf(buff2, "%d", &p);
          if (p < 0) return 1;
        } else {
          p = rank;
        ...
        if (p == rank) {
          if (gmsh_npe(atoi(data)) < 0) return 1;
          ...
          if (list_insertlast(list_elemv, (void *)&elemv)) return 1;
        }
      }
    }
    if (skip) skip = 0;
    ...
    if (sel == SN && !skip) {
      ...
      if (!gmsh_isvol(atoi(data), dim)) {
        ...
        if (gmsh_elems_belongs(list_elemv, &elems, &elems.elemv)) {
          if (list_insertlast(list_elems, (void *)&elems)) return 1;
        }
      }
    }
    if (skip) skip = 0;
...
int gmsh_readphys(char *file, list_t *list_elemv, list_t *list_elems, list_t *list_phys) {
  ...
  if (!file || !list_elemv || !list_elems || !list_phys) return 1;
  fm = fopen(file, "r");
  if (!fm) return 1;
  while (fgets(buff1, MAXC, fm)) {
    ...
    if (sel == SN && !skip) {
      ...
      if (list_insertlast(list_phys, (void *)&phys)) return 1;
      //}
    }
    if (skip) skip = 0;
  }
...
int gmsh_detghosts(list_t *list_nodes, list_t *list_elemv, list_t *list_ghost) {
  ...
  int e, n, i, fl;
  if (!list_nodes || !list_elemv || !list_ghost) return 1;
...
int gmsh_readghosts(char *file, list_t *list_ghost) {
  ...
  if (!file || !list_ghost) return 1;
  if (!list_ghost->sizelist) return 0;
  fm = fopen(file, "r");
  if (!fm) return 1;
  ...
  while (fgets(buff1, MAXC, fm)) {
    ...
    if (!data) return 1;
    if (strcmp(data, "$Nodes") == 0) {
      if (!fgets(buff1, MAXC, fm)) return 1;
      sel = SN;
      skip = 1;
    } else if (strcmp(data, "$EndNodes") == 0) {
      fclose(fm);
    ...
    if (sel == SN && !skip) {
      if (atoi(data) == ghost->n) {
        ...
        onode = onode->next;
        if (onode) ghost = (gmshN_t *)onode->data;
      }
    }
    if (skip) skip = 0;
  }
...
int gmsh_reenumerate(char *nfile, int rank, list_t *list_nodes, list_t *list_ghost, int **loc2gold, int **loc2gnew,
                     int **npp, int nproc) {
  FILE *fn;
  ...
  if (nproc == 1) {
    if (list_ghost->sizelist) return 1;
    for (i = 0; i < list_nodes->sizelist; i++) {
      (*loc2gold)[i] = i;
      (*loc2gnew)[i] = i;
    }
...
int gmsh_phys_belongs(list_t *list_elemv, list_t *list_elems, gmshP_t *phys) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    while (node) {
      ...
      if (elem->gmshid == phys->gmshid) return 1;
      node = node->next;
    }
  }
  return 0;
