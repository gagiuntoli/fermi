...
int gmsh_read(char *file, char *efile, char *nfile, int rank, int dim,
  ...
  if (gmsh_readnodes(file, nproc, nfile, rank, list_nodes))
    return 1;
  if (gmsh_readelemv(file, nproc, efile, rank, dim, list_elemv))
    return 2;
  if (gmsh_readelems(file, dim, list_elemv, list_elems))
    return 3;
  if (gmsh_readphys(file, list_elemv, list_elems, list_phyce))
    return 4;
  if (gmsh_detghosts(list_nodes, list_elemv, list_ghost))
    return 5;
  if (gmsh_readghosts(file, list_ghost))
    return 6;
  if (gmsh_reenumerate(nfile, rank, list_nodes, list_ghost, loc2gold, loc2gnew,
                       npp, nproc))
    return 7;
  return 0;
}
...
int gmsh_readelemv(char *file, int nproc, char *efile, int rank, int dim,
                   list_t *list_elemv) {
...
int gmsh_readelems(char *file, int dim, list_t *list_elemv,
                   list_t *list_elems) {
...
int gmsh_reenumerate(char *nfile, int rank, list_t *list_nodes,
  ...
  (*loc2gold) =
      (int *)calloc((list_nodes->sizelist + list_ghost->sizelist), sizeof(int));
  (*loc2gnew) =
      (int *)calloc((list_nodes->sizelist + list_ghost->sizelist), sizeof(int));
