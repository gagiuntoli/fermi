...
BOUNDARY_CONDITIONS
  PARAMETERS
    VARIATION:         CONSTANT
    INITIAL:           INERTIAL, Value= 0.0 0.0 1.0  
    $FIX_PRESSURE: ON, ON_NODE= 1077, VALUE= 1.0 $ top 
...
class Alya_field:
  ...
  def write(self, file_name):
    ...
    file_names = [] 
    file_names.append(self.file_name+"_vols_types.alya") 
    file_names.append(self.file_name+"_vols_nodes.alya") 
    file_names.append(self.file_name+"_coords.alya") 
    file_names.append(self.file_name+"_faces_nodes.alya") 
    file_names.append(self.file_name+"_faces_ids.alya") 
  ...
  def __to_nastin(self, file_name):
    ...
    file_out = open(file_name+"_nsi.nsi.dat", "w")
    print >> file_out, NSI_NSIDAT
    file_out.close()

    file_out = open(file_name+"_nsi.ker.dat", "w")
    print >> file_out, KER_DAT % (int(0.01*self.NUMBER_OF_STEPS)) 
    file_out.close()

    file_out = open(file_name+"_nsi.post.alyadat", "w")
    print >> file_out, POST_PROS 
    file_out.close()

    file_out = open(file_name+"_nsi.dat", "w")
    print >> file_out, RUN_DATA % (file_name, self.NUMBER_OF_STEPS) 
    file_out.close()

    file_out = open(file_name+"_nsi.run", "w")
    print >> file_out, RUN % (file_name, file_name)
    file_out.close()
    
    os.system( "chmod +x %s" % (file_name+"_nsi.run") )
    os.system( "cp %s_dom.alya %s_nsi.dom.dat"% (file_name, file_name) )
