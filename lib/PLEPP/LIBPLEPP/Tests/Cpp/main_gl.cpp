...
int main(int argc, char** argv)
{
  ...
  if(commij_size>0)
  {
    ...
    if(commij != MPI_COMM_NULL)
    {
      ...
      {
        int rval = -1;
        CD.mpi_tbicpl(NULL, 0, &rval, 1, namej); 
        cout<<"|=>["<< namei <<"]: "<< rval;  
        cout<<"\n"; 
      }
      ...
      if(local_rank == 0)
      {
        GetMesh[appi].set_ple_data("/home/jmake/z2014/Cplng/MeshCplng/MeshCoarse/square", 1); // n_Vrtx=47, n_elems=74
        GetMesh[appj].set_ple_data("/home/jmake/z2014/Cplng/MeshCplng/MeshCoarse/disk", 1); // n_Vrtx=57, n_elems=88
        //GetMesh[appj].set_ple_data("/home/jmake/z2014/Cplng/MeshCplng/Mesh02/mesh02", 1);
        //GetMesh[appi].set_ple_data("/home/jmake/z2014/Cplng/MeshCplng/Mesh01/mesh01", 2);
      }
      MPI_Barrier(local_comm);  
      ...
      //for(int i=0; i<stride*n_dist_points; i++) var_ij[i] = i;

      int n_interior    = ple_locator_get_n_interior(locatorj);
      var_ji = new double[stride*n_interior];
      for(int i=0; i<stride*n_interior;    i++) var_ji[i] = -666;

      CD.__locator_exchange_point_var_recv_double__(locatorj, var_ji, stride, reverse); 

      for(int i=0; i<n_interior; i++) cout<< var_ji[i]  <<" "; 
      cout<<"\n";
